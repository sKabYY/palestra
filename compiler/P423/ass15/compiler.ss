...
(define (parse-scheme pgm)

  (define (var->uvar var)
    (unique-name (string->symbol (extract-root var))))

  (define (new-env) '())

  (define (extend-env env)
    (lambda (u*)
      (let loop ([u* (reverse u*)] [u^* '()] [env env])
        (if (null? u*)
            (values env u^*)
            (let* ([u (car u*)]
                   [u^ (var->uvar u)])
  ...
  (define (apply-env env u)
    (cond
      [(assq u env) => cdr]
      [else (error 'parse "unbound variable" u)]))

  (define (in-env env u) (not (not (assq u env))))

  (define (parse-datum x)
    (match x
      [() '()]
      [(,[a] . ,[d]) `(,a . ,d)]
      [#(,[e*] ...) `#(,e* ...)]
      [,i
       (guard (number? i))
       (unless (fixnum? i)
         (error 'parse-datum "not a fixnum" i))
       (unless (fixnum-range? i)
         (error 'parse-datum "not in fixnum-range" i))
       i]
      [,b (guard (boolean? b)) b]
      [,x (error 'parse-datum "expect constant" x)]))

  (define (parse x env)
    (match x
      [(,rt ,[rd*] ...) (guard (in-env env rt))
       `(,(apply-env env rt) ,rd* ...)]
      [(quote ,datum) `(quote ,(parse-datum datum))]
      [(if ,[p] ,[x1]) `(if ,p ,x1 (void))]
      [(if ,[p] ,[x1] ,[x2]) `(if ,p ,x1 ,x2)]
      [(begin ,[e*] ...) `(begin ,e* ...)]
      [(lambda ,[(extend-env env) -> new-env a*] ,body ...)
       `(lambda (,a* ...) ,(parse (make-begin body) new-env))]
      [(let ([,u* ,[e*]] ...) ,body ...)
       (let-values ([(new-env u^*) ((extend-env env) u*)])
         `(let ([,u^* ,e*] ...) ,(parse (make-begin body) new-env)))]
      [(letrec ([,u* ,e*] ...) ,body ...)
       (let-values ([(new-env u^*) ((extend-env env) u*)])
         `(letrec ([,u^* ,(map (lambda (e) (parse e new-env)) e*)] ...)
            ,(parse (make-begin body) new-env)))]
      [(set! ,[u] ,[e]) `(set! ,u ,e)]
      [(not ,[e]) `(if ,e '#f '#t)]
      [(and) '#t]
      [(and ,[e]) e]
      [(and ,[e] ,e* ...)
       `(if ,e ,(parse `(and ,e* ...) env) '#f)]
      [(or) '#f]
      [(or ,[e]) e]
      [(or ,[e] ,e* ...)
       (let ([t (unique-name 't)])
         `(let ([,t ,e])
            (if ,t ,t ,(parse `(or ,e* ...) env))))]
      [(,op ,[e*] ...) (guard (prim? op))
       `(,op ,e* ...)]
      [(,[rt] ,[rd*] ...) `(,rt ,rd* ...)]
      [,c
       (guard (or (number? c) (boolean? c) (null? c)))
       `(quote ,(parse-datum c))]
