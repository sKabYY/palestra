    ...
    (define-rewrite 'fv*
      (lambda (x)
        (match x
          ...
          [,x x])))
    (case pass-name
      [(source skip-used-name convert-complex-datum
        convert-assignments remove-anonymous-lambda)
       `(let ()
      ...
      [(specify-representation)
       `(let ()
          ...
          (define (nop) (void))
          (ptr->datum ,x))]
      [(remove-let verify-uil remove-complex-opera*)
       `(let ()
       ...
       `(let ()
          ...
          (call/cc
            (lambda (k)
              ...
              ,(rewrite-opnds x)))
          (ptr->datum ,return-value-register))]
      [else (error 'language-wrapper "no wrapper for" pass-name)])))
